...
function onClickEvent(correct, selected, answerListId) {
    ...
    /* Used moment.js here to display response recorded*/
    const ele = document.getElementById(`time-${answerListId}`);
    ele.innerText = `Answered on ${moment().format('MMMM Do YYYY, h:mm:ss a')}`;
...
function showQuestions(questions) {
    ...
    for(let i = 0; i < questions.length; i++){
        ...
        questionP.classList.add("fa-solid")
        questionP.classList.add("fa-clipboard-question")
        ...
        timeP.setAttribute('id', `time-${i}`);
        questionsDiv.append(timeP);
