...
const interval = setInterval(() => {
    ...
    const pop = document.querySelector("#" + getRandomUnwhackedHoleId());
    
    if (pop != null) {
        pop.classList.add("needs-whack");
    }
...
for(const id of getAllHoleIds()) {
    ...
    const eleme = document.querySelector("#" + id);
    eleme.addEventListener('click', () => {
        if (eleme.classList.contains('needs-whack')) {
            eleme.classList.remove('needs-whack');
            eleme.classList.add('animating-whack');
            setTimeout(() => {
                eleme.classList.remove('animating-whack');
            }, 500);
